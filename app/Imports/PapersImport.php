...
class PapersImport implements ToModel, WithHeadingRow
{
    ...
    */
    ...
    public function getPaper(array $rows)
    {
        ...
        DB::transaction(function () use($rows, &$paper) {
            ...
            foreach ($rows as $row) {
                ...
                if (!is_array($row['题目答案'])) {
                    $row['题目答案'] = array($row['题目答案']);
                }
                // post_id
                if (isset($row['分类'])) {
                    if (!is_int($row['分类'])) {
                        $row['分类'] = Post::query()->where('name', $row['分类'])->first();
                    }
                }
